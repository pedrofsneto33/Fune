...
export const POST = withAuth(async (req: NextRequest, { auth }) => {
  try {
    ...
    if (!holder_name || !holder_cpf || !total_value || !first_due_date) {
      return NextResponse.json(
        { error: 'Parâmetros obrigatórios ausentes (holder_name, holder_cpf, total_value, first_due_date).' },
        { status: 400 }
      );
    }
    ...
    if (contract_id) {
      await supabaseAdmin.from('payments').insert(paymentsToInsert);
    }
