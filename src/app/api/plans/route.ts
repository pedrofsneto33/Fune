...
export const POST = withAuth(async (req: NextRequest, { auth }) => {
  try {
    ...
    if (!name || name.trim().length < 2) return NextResponse.json({ error: 'Nome do plano e obrigatorio (minimo 2 caracteres)' }, { status: 400 });
    ...
    if (max_dependents !== undefined && (!Number.isInteger(Number(max_dependents)) || Number(max_dependents) < 0)) return NextResponse.json({ error: 'Limite de dependentes deve ser um numero inteiro positivo' }, { status: 400 });
...
export const PATCH = withAuth(async (req: NextRequest, { auth }) => {
  try {
    ...
    if (!id || !isValidUUID(id)) return NextResponse.json({ error: 'ID invalido' }, { status: 400 });
    ...
    if (error) { if ((error as any).code === 'PGRST116') return NextResponse.json({ error: 'Plano nao encontrado' }, { status: 404 }); return NextResponse.json({ error: 'Erro ao atualizar plano' }, { status: 500 }); }
    return NextResponse.json(data);
  } catch { return NextResponse.json({ error: 'Erro interno' }, { status: 500 }); }
}, ['superadmin', 'admin', 'manager']);
...
export const DELETE = withAuth(async (req: NextRequest, { auth }) => {
  try {
    ...
    if (checkErr) return NextResponse.json({ error: 'Erro ao verificar dependencias' }, { status: 500 });
    if (active && active.length > 0) return NextResponse.json({ error: 'Nao e possivel excluir: ha contratos ativos vinculados a este plano.' }, { status: 409 });
