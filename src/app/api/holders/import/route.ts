...
export const POST = withAuth(async (req: NextRequest, { auth }) => {
  try {
    ...
    if (!rawCsv.trim()) {
      return NextResponse.json({ error: 'Conteudo CSV vazio.' }, { status: 400 });
    }

    const lines = rawCsv.split(/\r?\n/).filter((l: string) => l.trim().length > 0);
    if (lines.length > MAX_LINES) {
      return NextResponse.json({ error: 'Limite de 1000 linhas por importacao.' }, { status: 400 });
    }
    ...
    lines.forEach((line: string, idx: number) => {
      ...
      if (lineNo === 1 && /nome/i.test(cols[0] || '') && /cpf/i.test(cols[1] || '')) return;
      ...
      if (cpf.length !== 11) {
        invalid.push({ line: lineNo, reason: 'CPF invalido (precisa ter 11 digitos)' });
        return;
      }
    ...
    if (parsed.length === 0) {
      return NextResponse.json({ success: true, imported: 0, skipped_duplicates: 0, invalid });
    }
    ...
    const { data: existing } = await supabaseAdmin
      ...
      .eq('tenant_id', auth.tenantId)
      .in('cpf', cpfList);
    const existingSet = new Set((existing || []).map((e: { cpf: string }) => e.cpf));
    ...
    overflow.forEach((p) => {
      invalid.push({ line: p.line, reason: 'Nao importado: limite do plano atingido' });
    });
    ...
    for (let i = 0; i < toInsert.length; i += CHUNK_SIZE) {
      ...
      if (error) {
        chunk.forEach((_, cIdx) => {
          invalid.push({ line: toInsert[i + cIdx].line, reason: 'Erro no banco: ' + error.message });
        });
        continue;
      }
    ...
    if (insertedIds.length > 0) {
      ...
      if (!planId) {
        const { data: newP } = await supabaseAdmin
          ...
          .insert([{
            ...
            monthly_fee: clampNumber(69.90, 0, 10000),
            max_dependents: 6,
          }])
          .select('id')
          .single();
      ...
      if (planId) {
        ...
        for (let i = 0; i < contracts.length; i += CHUNK_SIZE) {
          await supabaseAdmin.from('contracts').insert(contracts.slice(i, i + CHUNK_SIZE));
        }
      }
    }
