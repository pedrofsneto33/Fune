    ...
    <main className="min-h-screen bg-[#070b14] text-slate-900 dark:text-slate-100 selection:bg-blue-500/40">
        ...
        <div className="max-w-6xl mx-auto px-5 h-16 flex items-center justify-between">
          <div className="flex items-center gap-2.5">
            <div className="w-9 h-9 rounded-xl bg-gradient-to-br from-blue-500 to-blue-700 flex items-center justify-center shadow-lg shadow-blue-500/25">
              <HeartPulse className="w-5 h-5 text-slate-900 dark:text-white" />
            </div>
            <div className="leading-none">
              <span className="font-bold text-base tracking-tight">Eternity<span className="text-blue-400">OS</span></span>
              <span className="block text-[10px] text-slate-600 dark:text-slate-500 dark:text-slate-400">by PrimeX Sistemas</span>
            </div>
          </div>
      ...
      <section className="relative overflow-hidden">
        ...
        <div className="relative max-w-6xl mx-auto px-5 pt-20 pb-24 text-center">
          ...
          <p className="max-w-2xl mx-auto mt-6 text-slate-600 dark:text-slate-500 dark:text-slate-400 text-base md:text-lg leading-relaxed">
            De associados e carteirinhas ao plantão 24h, cobrança, frota e financeiro — o EternityOS automatiza a operação inteira para você cuidar do que importa: <span className="text-slate-700 dark:text-slate-200 font-medium">as famílias atendidas.</span>
          </p>
          ...
          <div className="flex flex-wrap items-center justify-center gap-x-8 gap-y-3 mt-12 text-xs text-slate-600 dark:text-slate-500">
      ...
      <section id="recursos" className="max-w-6xl mx-auto px-5 py-20 border-t border-white/5">
        <div className="text-center mb-14">
          ...
          <p className="text-slate-600 dark:text-slate-500 dark:text-slate-400 mt-3 max-w-xl mx-auto">Módulos completos e integrados — nada de planilhas soltas e cadernos de plantão.</p>
        </div>
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-5">
          {MODULES.map((m) => (
            <div key={m.title} className="group rounded-2xl border border-white/8 bg-white/[0.03] p-6 hover:border-blue-500/40 hover:bg-blue-500/[0.04] transition-all duration-300">
              ...
              <h3 className="font-bold text-slate-900 dark:text-white mb-2">{m.title}</h3>
              <p className="text-sm text-slate-600 dark:text-slate-500 dark:text-slate-400 leading-relaxed">{m.text}</p>
            </div>
          ))}
        </div>
      </section>
      ...
      <section id="planos" className="max-w-6xl mx-auto px-5 py-20 border-t border-white/5">
        <div className="text-center mb-14">
          ...
          <p className="text-slate-600 dark:text-slate-500 dark:text-slate-400 mt-3">Mensalidade previsível, sem surpresa. Troque de plano quando quiser.</p>
        </div>
        <div className="grid lg:grid-cols-3 gap-6 items-stretch">
          {PLANS.map((p) => (
            <div key={p.code} className={`relative rounded-2xl border p-7 flex flex-col ${p.badge ? 'border-blue-500/50 bg-gradient-to-b from-blue-500/10 to-transparent shadow-2xl shadow-blue-900/40' : 'border-white/8 bg-white/[0.03]'}`}>
              {p.badge && <span className="absolute -top-3 left-1/2 -translate-x-1/2 text-[11px] font-bold uppercase tracking-wide bg-blue-600 text-white dark:text-white rounded-full px-3.5 py-1 shadow-lg shadow-blue-600/40">{p.badge}</span>}
              <h3 className="font-bold text-lg text-slate-900 dark:text-white">{p.name}</h3>
              <p className="text-xs text-slate-600 dark:text-slate-500 dark:text-slate-400 mt-1 mb-5">{p.desc}</p>
              <div className="mb-6">
                <span className="text-4xl font-extrabold tracking-tight">{p.price}</span>
                <span className="text-slate-600 dark:text-slate-500 dark:text-slate-400 text-sm">{p.period}</span>
              </div>
        ...
        <p className="text-center text-xs text-slate-600 dark:text-slate-500 mt-8">Limites por plano: associados e usuários monitorados automaticamente no painel. Enterprise sem limites.</p>
      </section>
        ...
        <div className="space-y-3">
          {FAQ.map((f, idx) => (
            <div key={f.q} className="rounded-xl border border-white/8 bg-white/[0.03] overflow-hidden">
              <button onClick={() => setOpenFaq(openFaq === idx ? null : idx)} className="w-full flex items-center justify-between gap-4 px-5 py-4 text-left text-sm font-semibold text-slate-900 dark:text-slate-100 hover:bg-white/[0.03] transition">
                {f.q}
                <ChevronDown className={`w-4 h-4 shrink-0 text-slate-600 dark:text-slate-500 dark:text-slate-400 transition-transform ${openFaq === idx ? 'rotate-180' : ''}`} />
              </button>
              {openFaq === idx && <p className="px-5 pb-4 text-sm text-slate-600 dark:text-slate-500 dark:text-slate-400 leading-relaxed">{f.a}</p>}
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="relative overflow-hidden border-t border-white/5">
        ...
        <div className="relative max-w-4xl mx-auto px-5 py-20 text-center">
          ...
          <p className="text-slate-600 dark:text-slate-500 dark:text-slate-400 mt-4 max-w-xl mx-auto">Fale agora com a PrimeX Sistemas, veja o sistema ao vivo e comece ainda esta semana.</p>
      ...
      <footer className="border-t border-white/5 bg-black/30">
        <div className="max-w-6xl mx-auto px-5 py-12 grid gap-10 md:grid-cols-3">
          <div>
            ...
            <p className="text-sm text-slate-600 dark:text-slate-500 dark:text-slate-400 leading-relaxed">Software de gestão completo para funerárias, planos funerários e serviços de assistência familiar.</p>
          </div>
          <div>
            <h4 className="text-sm font-bold text-slate-900 dark:text-white mb-4 flex items-center gap-2"><Phone className="w-4 h-4 text-blue-400" /> Contato</h4>
            <ul className="space-y-3 text-sm text-slate-600 dark:text-slate-500 dark:text-slate-400">
              <li>
              ...
              <li className="flex items-center gap-2"><MapPin className="w-4 h-4 text-slate-600 dark:text-slate-500" /> Teresina - PI, Brasil</li>
            </ul>
          </div>
          <div>
            <h4 className="text-sm font-bold text-slate-900 dark:text-white mb-4 flex items-center gap-2"><Building2 className="w-4 h-4 text-blue-400" /> Acesso</h4>
            <ul className="space-y-3 text-sm text-slate-600 dark:text-slate-500 dark:text-slate-400">
