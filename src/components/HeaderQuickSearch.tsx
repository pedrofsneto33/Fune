    ...
    <div className="relative w-full max-w-lg">
      <div className="relative flex items-center">
        <input
          ...
          className="w-full pl-10 pr-4 py-2 text-sm bg-slate-800 border border-slate-700 text-white placeholder-slate-400 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 shadow-inner"
        />
      ...
      {results.length > 0 && (
        <div className="absolute left-0 right-0 mt-2 bg-slate-900 border border-slate-700 rounded-lg shadow-2xl z-50 overflow-hidden">
          {results.map((holder) => {
            ...
            return (
              <div
                ...
                className="p-3 hover:bg-slate-800 border-b border-slate-800 cursor-pointer transition-colors flex items-center justify-between text-left"
              >
                <div>
                  <p className="text-sm font-semibold text-white">{holder.full_name}</p>
                  <p className="text-xs text-slate-400">CPF: {holder.cpf} • Plano: {contract?.plans?.name || 'Sem plano'}</p>
                </div>
                <span
        ...
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center p-4 z-50">
          <div className="bg-slate-900 border border-slate-700 rounded-xl max-w-lg w-full p-6 text-white shadow-2xl">
            <div className="flex justify-between items-start border-b border-slate-800 pb-3">
              <div>
                <h3 className="text-lg font-bold text-white">{selectedHolder.full_name}</h3>
                <p className="text-xs text-slate-400">CPF: {selectedHolder.cpf} • Tel: {selectedHolder.phone}</p>
              </div>
              <button
            ...
            <div className="mt-4 space-y-3 text-sm">
              ...
              <div>
                <p className="font-semibold text-xs text-slate-400 uppercase tracking-wider mb-1">Dependentes Cobertos ({selectedHolder.dependents?.length || 0}):</p>
                <div className="max-h-32 overflow-y-auto space-y-1">
                  {(selectedHolder.dependents || []).map((dep) => (
                    <div key={dep.id} className="text-xs bg-slate-950 p-2 rounded flex justify-between">
                      <span>{dep.full_name}</span>
                      <span className="text-slate-500">({dep.relation})</span>
                    </div>
                  ))}
            ...
            <div className="mt-6 flex justify-end gap-2">
              <button
                onClick={() => setSelectedHolder(null)}
                className="px-4 py-2 text-xs text-slate-400 hover:text-white rounded bg-slate-800"
              >
                Fechar
              </button>
              <button
