  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4">
      <div className="relative w-full max-w-2xl bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-2xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
        ...
        {/* Header */}
        <div className="p-6 border-b border-zinc-200 dark:border-zinc-800 flex items-center justify-between bg-slate-50 dark:bg-zinc-950/40">
          <div className="flex items-center gap-3">
            <div className="p-2.5 bg-blue-500/10 border border-blue-500/20 rounded-xl text-blue-400">
              <Users className="w-5 h-5" />
            </div>
            <div>
              <h2 className="text-base font-bold text-slate-900 dark:text-white tracking-wide">Dependentes do Associado</h2>
              <p className="text-xs text-zinc-400">
                Titular: <span className="text-zinc-800 dark:text-zinc-200 font-medium">{holder.holder}</span> ({holder.cpf}) — Plano: <span className="text-emerald-400 font-medium">{holder.plan}</span>
              </p>
            </div>
          </div>
          <button
            onClick={onClose}
            className="p-2 text-zinc-400 hover:text-zinc-200 hover:bg-zinc-200 dark:hover:bg-zinc-800 rounded-lg transition"
          >
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        <div className="p-6 space-y-6 overflow-y-auto">
          ...
          {/* Form de Adição */}
          <form onSubmit={onAddDependent} className="bg-slate-50 dark:bg-zinc-950/60 p-4 rounded-xl border border-zinc-200 dark:border-zinc-800/80 space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
              <div className="md:col-span-1">
                ...
                <input
                  ...
                  className="w-full bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-700 rounded-lg px-3 py-2 text-xs text-zinc-900 dark:text-zinc-100 placeholder-zinc-500 focus:outline-none focus:border-blue-500"
                />
              </div>
              ...
              <div>
                ...
                <select
                  ...
                  className="w-full bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-700 rounded-lg px-3 py-2 text-xs text-zinc-900 dark:text-zinc-100 focus:outline-none focus:border-blue-500"
                >
            ...
            <div className="flex justify-end pt-1">
              <button
                type="submit"
                className="px-4 py-2 bg-emerald-600 hover:bg-emerald-500 text-white dark:text-white rounded-lg text-xs font-semibold flex items-center gap-2 shadow-lg shadow-emerald-950/40 transition"
              >
            ...
            {dependents.length === 0 ? (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                {dependents.map((dep) => (
                  <div
                    key={dep.id}
                    className="bg-slate-50 dark:bg-zinc-950/40 border border-zinc-200 dark:border-zinc-800/80 p-3.5 rounded-xl flex items-center justify-between"
                  >
                    <div>
                      <p className="text-xs font-bold text-slate-900 dark:text-white">{dep.full_name}</p>
        ...
        <div className="p-4 border-t border-zinc-200 dark:border-zinc-800 bg-slate-50 dark:bg-zinc-950/40 flex justify-end">
          <button
            onClick={onClose}
            className="px-4 py-2 bg-slate-100 dark:bg-zinc-800 hover:bg-zinc-700 text-zinc-800 dark:text-zinc-200 rounded-lg text-xs font-medium transition"
          >
            Fechar
          </button>
        </div>
