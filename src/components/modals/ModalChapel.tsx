        ...
        <form onSubmit={handleSubmit} className="space-y-3">
          <div>
            ...
            <select value={chapelName} onChange={e => setChapelName(e.target.value)} className="w-full bg-zinc-950 border border-zinc-700 rounded-xl px-3 py-2 text-xs text-white">
              <option value="Capela Memorial 01">Capela Memorial 01</option>
              <option value="Capela Pax Teresina 02">Capela Pax Teresina 02</option>
              <option value="Salão Nobre VIP">Salão Nobre VIP</option>
            </select>
          </div>
          <div>
          ...
          <div>
            ...
            <input required type="text" value={familyContact} onChange={e => setFamilyContact(e.target.value)} className="w-full bg-zinc-950 border border-zinc-700 rounded-xl px-3 py-2 text-xs text-white" />
          </div>
          <button disabled={loading} className="w-full py-2.5 bg-blue-600 hover:bg-blue-500 text-white rounded-xl text-xs font-bold transition">
            {loading ? 'Agendando...' : 'Confirmar Reserva de Capela'}
          </button>
        </form>
      </div>
    </div>
