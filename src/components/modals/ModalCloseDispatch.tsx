...
export function ModalCloseDispatch({
  ...
  return (
    <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center p-4 z-50 overflow-y-auto">
      <div className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 w-full max-w-lg rounded-2xl p-6 shadow-2xl space-y-4 my-8">
        <div className="flex justify-between items-center pb-3 border-b border-zinc-200 dark:border-zinc-800">
          <div className="flex items-center gap-2">
            <Truck className="w-5 h-5 text-emerald-400" />
            <div>
              <h3 className="text-base font-bold text-slate-900 dark:text-white">Retorno à Base & Fechamento de OS</h3>
        ...
        <form onSubmit={handleSubmit} className="space-y-4 text-xs">
          ...
          <div className="p-3 bg-slate-50 dark:bg-zinc-950 rounded-xl border border-zinc-200 dark:border-zinc-800/80 grid grid-cols-2 gap-2 text-zinc-700 dark:text-zinc-300">
            <div>
              <span className="text-zinc-500 block text-[10px] uppercase">Veículo / Placa</span>
              <strong className="text-slate-900 dark:text-white text-xs">{dispatch.vehicle_plate || dispatch.vehicle || 'Veículo Designado'}</strong>
            </div>
            <div>
              <span className="text-zinc-500 block text-[10px] uppercase">Motorista / Agente</span>
              <strong className="text-slate-900 dark:text-white text-xs">{dispatch.driver_agent || dispatch.driverName || 'Não informado'}</strong>
            </div>
            <div className="col-span-2 pt-1 border-t border-zinc-200 dark:border-zinc-800/50 flex justify-between items-center">
              <span className="text-zinc-400">Odômetro de Saída:</span>
              <span className="font-mono text-slate-900 dark:text-white font-bold">{odoStart.toLocaleString('pt-BR')} KM</span>
            </div>
          </div>

          {/* Odômetro Final */}
          <div>
            <label className="block text-zinc-700 dark:text-zinc-300 font-semibold mb-1 flex items-center gap-1.5">
            ...
            <input
              ...
              className="w-full bg-slate-100 dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 rounded-lg px-3 py-2 text-slate-900 dark:text-white focus:outline-none focus:border-blue-500 font-mono text-sm"
            />
          </div>
          ...
          <div className="grid grid-cols-2 gap-3 pt-1">
            <div>
              ...
              <input
                ...
                className="w-full bg-slate-100 dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 rounded-lg px-3 py-2 text-slate-900 dark:text-white focus:outline-none focus:border-blue-500"
              />
            </div>
            <div>
              <label className="block text-zinc-700 dark:text-zinc-300 font-semibold mb-1">Custo Abastecimento (R$)</label>
              <input
                type="number"
                step="0.01"
                min="0"
          ...
          <div>
            <label className="block text-zinc-700 dark:text-zinc-300 font-semibold mb-1">Observações / Avarias / Ocorrências</label>
            <textarea
              rows={2}
              placeholder="Ex: Veículo retornado limpo, sem avarias. Atendimento finalizado no cemitério municipal."
              value={notes}
              onChange={(e) => setNotes(e.target.value)}
              className="w-full bg-slate-100 dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 rounded-lg px-3 py-2 text-slate-900 dark:text-white focus:outline-none focus:border-blue-500 resize-none text-xs"
            />
          </div>
          ...
          <div className="flex justify-end gap-3 pt-3 border-t border-zinc-200 dark:border-zinc-800">
            ...
            <button
              ...
              className="px-4 py-2 rounded-xl bg-emerald-600 hover:bg-emerald-500 text-white dark:text-white font-semibold disabled:opacity-50 flex items-center gap-2 shadow-lg shadow-emerald-600/20"
            >
