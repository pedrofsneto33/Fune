      ...
      <div className="relative w-full max-w-3xl bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-2xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
        ...
        <div className="p-5 border-b border-zinc-200 dark:border-zinc-800 flex items-center justify-between bg-slate-50 dark:bg-zinc-950/40">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-base font-bold text-slate-900 dark:text-white tracking-wide">Régua de Cobrança WhatsApp em Massa</h2>
        ...
        <div className="p-6 space-y-5 overflow-y-auto">
          ...
          <div className="space-y-1.5">
            <label className="text-xs font-semibold text-zinc-700 dark:text-zinc-300">Modelo da Mensagem (Tags dinâmicas: {`{nome}`}, {`{plano}`}, {`{valor}`}, {`{vencimento}`})</label>
            <textarea
              rows={3}
              value={customMsgTemplate}
              onChange={(e) => setCustomMsgTemplate(e.target.value)}
              className="w-full bg-slate-50 dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-700 rounded-xl p-3 text-xs text-zinc-900 dark:text-zinc-100 focus:outline-none focus:border-green-500 font-sans"
            />
          </div>
          ...
          <div className="space-y-3">
            ...
            {pendingList.length === 0 ? (
              <div className="p-8 text-center bg-slate-50 dark:bg-zinc-950/40 border border-zinc-200 dark:border-zinc-800 rounded-xl">
                <CheckCircle2 className="w-8 h-8 text-emerald-400 mx-auto mb-2" />
                <p className="text-xs text-zinc-700 dark:text-zinc-300 font-semibold">Tudo em dia! Nenhum associado pendente no momento.</p>
              </div>
            ) : (
                  ...
                  return (
                    <div
                      ...
                      className={`p-3.5 rounded-xl border flex items-center justify-between transition ${
                        isSent
                          ? 'bg-slate-50 dark:bg-zinc-950/40 border-zinc-200 dark:border-zinc-800 opacity-60'
                          : isChecked
                          ? 'bg-slate-50 dark:bg-zinc-950/80 border-green-500/40'
                          : 'bg-slate-50 dark:bg-zinc-950/40 border-zinc-200 dark:border-zinc-800'
                      }`}
                    >
                      <div className="flex items-center gap-3">
                        <input
                          ...
                          className="rounded border-zinc-200 dark:border-zinc-700 text-green-600 focus:ring-0 w-4 h-4 bg-white dark:bg-zinc-900 cursor-pointer"
                        />
                        <div>
                          <p className="font-bold text-slate-900 dark:text-white text-xs">{item.holder}</p>
                        ...
                        <button
                          ...
                          className={`px-3 py-1.5 rounded-lg text-xs font-semibold flex items-center gap-1.5 transition shadow ${
                            isSent
                              ...
                              : 'bg-green-600 hover:bg-green-500 text-white dark:text-white shadow-green-950/40'
                          }`}
                        >
