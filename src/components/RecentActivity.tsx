...
export default function RecentActivity({ role }: { role?: AppRole | null }) {
  ...
  if (loading) {
    return (
      <section className="bg-[#0d111a] border border-slate-800 rounded-lg p-4">
        <p className="text-slate-400 text-xs">Carregando atividade…</p>
      </section>
    );
  }
    ...
    <section className="bg-[#0d111a] border border-slate-800 rounded-lg p-4">
      <div className="flex items-center justify-between mb-3">
        ...
        {role && isTabAllowed(role, 'audit') && (
          <Link href="/auditoria" className="text-xs text-emerald-400 hover:text-emerald-300">
            Ver tudo →
          </Link>
        )}
      </div>
        ...
        <ul className="space-y-2">
          {logs.map((log) => (
            <li key={log.id} className="flex items-center gap-3 text-xs">
              ...
              <span className="text-slate-300 truncate">{log.user_email}</span>
              <span className="text-slate-400 truncate ml-auto">{log.action}</span>
            </li>
          ))}
        </ul>
      )}
