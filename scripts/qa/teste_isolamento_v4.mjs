...
console.log('\n=== QA ISOLAMENTO DEFINITIVO v4 ===\n');

// Setup: criar holders com cpf (NOT NULL)
const ownHolder = (await admin.from('holders').insert({ full_name: 'QA Own Holder', cpf: '111.111.111-11', tenant_id: TENANT_QA }).select('id').single()).data;
const otherHolder = (await admin.from('holders').insert({ full_name: 'QA Other Holder', cpf: '222.222.222-22', tenant_id: TENANT_OTHER }).select('id').single()).data;
