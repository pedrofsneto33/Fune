...
console.log('\n=== QA ISOLAMENTO DEFINITIVO v3 ===\n');

// Setup: criar holders (sem created_by)
const ownHolder = (await admin.from('holders').insert({ full_name: 'QA Own Holder', tenant_id: TENANT_QA }).select('id').single()).data;
const otherHolder = (await admin.from('holders').insert({ full_name: 'QA Other Holder', tenant_id: TENANT_OTHER }).select('id').single()).data;
